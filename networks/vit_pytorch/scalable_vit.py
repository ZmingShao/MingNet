...
class InteractiveWindowedSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        assert (height % wsz_h) == 0 and (
                    width % wsz_w) == 0, f'height ({height}) or width ({width}) of feature map is not divisible by the window size ({wsz_h}, {wsz_w})'
        ...
        q, k, v = map(
            lambda t: rearrange(t, 'b (h d) (x w1) (y w2) -> (b x y) h (w1 w2) d', h=heads, w1=wsz_h, w2=wsz_w),
            (q, k, v))
        ...
        out = rearrange(out, '(b x y) h (w1 w2) d -> b (h d) (x w1) (y w2)', x=height // wsz_h, y=width // wsz_w,
                        w1=wsz_h, w2=wsz_w)
    ...
    def __init__(
        ...
        for ind in range(depth):
            ...
            self.layers.append(nn.ModuleList([
                ...
                PreNorm(dim, FeedForward(dim, expansion_factor=ff_expansion_factor, dropout=dropout)),
                PreNorm(dim, InteractiveWindowedSelfAttention(dim, heads=heads, dim_key=iwsa_dim_key,
                                                              dim_value=iwsa_dim_value, window_size=iwsa_window_size,
                                                              dropout=dropout))
            ]))
...
class ScalableViT(nn.Module):
    def __init__(
        ...
        for ind, (
        layer_dim, layer_depth, layer_heads, layer_ssa_dim_key, layer_ssa_dim_value, layer_ssa_reduction_factor,
        layer_iwsa_dim_key, layer_iwsa_dim_value, layer_window_size) in enumerate(
                zip(dims, depth, *hyperparams_per_stage)):
            ...
            self.layers.append(nn.ModuleList([
                Transformer(dim=layer_dim, depth=layer_depth, heads=layer_heads,
                            ...
                            iwsa_window_size=layer_window_size, norm_output=not is_last),

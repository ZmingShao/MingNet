...
class SegmentationNetwork(NeuralNetwork):
    def __init__(self):
        ...
        self.n_classes = None  # number of channels in the output
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if all_in_gpu:
            ...
            aggregated_results = torch.zeros([self.n_classes] + list(data.shape[1:]), dtype=torch.half,
            ...
            aggregated_nb_of_predictions = torch.zeros([self.n_classes] + list(data.shape[1:]), dtype=torch.half,
                                                       device=self.get_device())
        else:
            if use_gaussian and num_tiles > 1:
            ...
            aggregated_results = np.zeros([self.n_classes] + list(data.shape[1:]), dtype=np.float32)
            aggregated_nb_of_predictions = np.zeros([self.n_classes] + list(data.shape[1:]), dtype=np.float32)
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        result_torch = torch.zeros([1, self.n_classes] + list(x.shape[2:]),
    ...
    def _internal_maybe_mirror_and_pred_2D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        x = to_cuda(maybe_to_torch(x), gpu_id=self.get_device())
        result_torch = torch.zeros([x.shape[0], self.n_classes] + list(x.shape[2:]),
                                   dtype=torch.float).cuda(self.get_device(), non_blocking=True)
